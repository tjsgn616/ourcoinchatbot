...
def acc():
    ...
    # 실시간 변동률 구하기
    top_change_info = []
    # print(type(top_change['market']))
    # print(type(list(top_change['market'])))
    for k in list(top_change['market']):
        all_url = (f"https://api.upbit.com/v1/ticker?markets={k}")
        all_headers = {"Accept": "application/json"}
        a = requests.get(all_url, headers=all_headers).json()
        top_change_info.extend(a)
    # print(top_change_info[0],type(top_change_info[0]))
    # print(top_change_info[0]['market'])
    top_change_rate = []
    top_change_str = []
    for i in range(5):
        top_change_rate.append(round(top_change_info[i]['change_rate']*100,2))
        if top_change_info[i]['change'] == "RISE":
            top_change_str.append("상승")
        elif top_change_info[i]['change'] == "FALL":
            top_change_str.append("하락")
        else:
            top_change_str.append("보합")
    print(top_change_rate[0].type(top_change_rate))
                    ...
                    },
                {
                    ...
                    "items": [
                        ...
                        "items": [
                            ...
                            "description":  f"{top_change_rate}-{top_change_str}",
